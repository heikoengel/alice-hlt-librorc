    ...
    uint32_t *eventbuffer)
{
  ...
  for(i=0;i<reportbuffer[rb_index].calc_event_size;i++) {
    fprintf(fd_log, "%03ld: %08x\n",
        i, (uint32_t)*(eventbuffer + reportbuffer[rb_index].offset + i));
  }

#if DMA_MODE==32
  fprintf(fd_log, "%03ld: EOE reported_event_size: %08x\n", i,
      (uint32_t)*(eventbuffer + reportbuffer[rb_index].offset + i));
#endif
#if DMA_MODE==128
  //TODO: the following is buggy!!!
  i++;
  printf("%03ld: EOE calc_event_size: %08x\n", i,
      (uint32_t)*(eventbuffer + reportbuffer[rb_index].offset + i));
  i++;
  ...
  result = fwrite(eventbuffer + reportbuffer[rb_index].offset, 4,
