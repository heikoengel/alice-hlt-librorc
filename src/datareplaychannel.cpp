...
namespace LIBRARY_NAME
{
    ...
    {
        uint32_t chsts = m_link->GTX(RORC_REG_DDR3_DATA_REPLAY_CHANNEL_STS);
        return ((chsts & (1<<0)) != 0);
    }
    ...
    {
        uint32_t chctrl = m_link->GTX(RORC_REG_DDR3_DATA_REPLAY_CHANNEL_CTRL);
        return ((chctrl & (1<<2)) != 0);
    }
    ...
    )
    {
        ...
        chctrl &= ~(1<<0);
        chctrl |= ((enable&1)<<0);
        m_link->setGTX(RORC_REG_DDR3_DATA_REPLAY_CHANNEL_CTRL, chctrl);
    }
    ...
    {
        return (m_link->GTX(RORC_REG_DDR3_DATA_REPLAY_CHANNEL_STS) &
                0x7ffffff8);
    }
    ...
    {
        return (m_link->GTX(RORC_REG_DDR3_DATA_REPLAY_CHANNEL_CTRL) &
                0x7ffffff8);
    }
    ...
    {
        uint32_t ch_cfg = m_link->GTX(RORC_REG_DDR3_DATA_REPLAY_CHANNEL_CTRL);
        ch_cfg &= ~(0x7ffffff8); // clear [30:3]
        ch_cfg |= (start_address & 0x7ffffff8); //set start_address[30:3]
        m_link->setGTX(RORC_REG_DDR3_DATA_REPLAY_CHANNEL_CTRL, ch_cfg);
    }
